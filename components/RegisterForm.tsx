...
export function RegisterForm({ googleOAuthEnabled = false }: { googleOAuthEnabled?: boolean }) {
  ...
  return (
    <form action={formAction} className="pr-card grid gap-4 p-6">
      ...
      {state.error ? (
        <p className="rounded-[var(--radius)] border border-[rgba(196,43,43,0.2)] bg-[var(--danger-50)] px-4 py-3 text-sm font-[800] text-[var(--danger)]" role="alert">
          {state.error}
        </p>
      ) : null}
