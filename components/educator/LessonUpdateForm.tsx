  ...
  return (
    <details className="rounded-[var(--radius)] border border-[var(--border)] bg-white">
      ...
      <form
        ...
        onSubmit={(event) => {
          ...
          startTransition(async () => {
            ...
            setMessage(result.ok ? "Lesson updated. Submit the course again when ready." : result.error);
      ...
      >
        ...
        <div className="grid gap-2 sm:grid-cols-2">
          <input className="pr-input" value={form.descriptionEn} onChange={(e) => setForm({ ...form, descriptionEn: e.target.value })} placeholder="English description" />
          <input className="pr-input" value={form.descriptionPs} onChange={(e) => setForm({ ...form, descriptionPs: e.target.value })} placeholder="Pashto description" />
        </div>
        {lesson.type === LessonType.VIDEO ? (
          <input className="pr-input" value={form.youtubeUrl} onChange={(e) => setForm({ ...form, youtubeUrl: e.target.value })} placeholder="YouTube URL" />
        ) : null}
        {lesson.type === LessonType.READING ? (
          <div className="grid gap-3">
            ...
            <div className="grid gap-2 sm:grid-cols-2">
              <textarea className="pr-input min-h-40" value={form.readingEn} onChange={(e) => setForm({ ...form, readingEn: e.target.value })} placeholder="English reading content" />
              <textarea className="pr-input min-h-40" value={form.readingPs} onChange={(e) => setForm({ ...form, readingPs: e.target.value })} placeholder="Pashto reading content" />
            </div>
            {form.readingEn ? (
